...
$.extend($, {
    ...
    // * @param timer
    // */
    //exportData: function(data, interval, timer){
    //    if(!data) {
    //        $('#load').hide();
    //        interval.cancel(timer);
    //    }else{
    //        $('#load').show();
    //        $('#loading p').css({width: data.rate+'%'});
    //        var loadingW = Number($('#loading').width());
    //        var spanW = Number($('#loading span').width());
    //        var paramsW = ((loadingW - spanW)/100) * Number(data.rate);
    //        $("#progressValue").text(data.rate +" %");
    //        $('#loading span').css({ left: paramsW +'px' });
